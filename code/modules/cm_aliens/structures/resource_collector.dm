...
/obj/effect/alien/resin/collector/Initialize(mapload, hive_ref, new_node)
	...
	if(hive_ref)
		...
		if(faction.living_xeno_queen)
			...
			xeno_message("Hive: \A [src] has been constructed at [sanitize(current_area_name)]!", 3, faction)
	if(new_node)
...
/obj/effect/alien/resin/collector/process()
	if(!faction || world.time < (last_gathered_time + gather_cooldown))
		return
	var/current_area_name = get_area_name(src)
	if(!connected_node)
		visible_message(SPAN_DANGER("\The [src] groans and collapses as its contents are reduced to nothing!"))
		if(faction.living_xeno_queen)
			xeno_message("Hive: \A [src] has been depleted at [sanitize(current_area_name)]!", 3, faction)
		qdel(src)
		return
	last_gathered_time = world.time
	faction.crystal_stored += connected_node.gather_resource()
	faction.xenocon_points += COLLECTOR_XENOCON_RATE
