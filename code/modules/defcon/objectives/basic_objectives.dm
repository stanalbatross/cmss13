...
/datum/cm_goals/retrieve_item/New(var/T)
	..()
	if(T)
		target_item = T
...
/datum/cm_goals/retrieve_item/Destroy()
	target_item = null
	target_areas = null
	initial_location = null
	return ..()

/datum/cm_goals/retrieve_item/proc/clean_up_ref()
	SIGNAL_HANDLER
	target_item = null
	fail()

/datum/cm_goals/retrieve_item/get_clue()
	return SPAN_DANGER("[target_item] in <u>[initial_location]</u>")

/datum/cm_goals/retrieve_item/check_completion()
	. = ..()
	if(!target_item)
		fail()
		return FALSE
...
/datum/cm_goals/retrieve_item/almayer
	target_areas = list(
...
/datum/cm_goals/communications/complete()
	. = ..()
...
/datum/cm_goals/communications/uncomplete()
	if(complete)
		last_contact_time = world.time
		addtimer(CALLBACK(src, .proc/blackout_warning, last_contact_time), 45 SECONDS + rand(0, 45 SECONDS))
	. = ..()

/datum/cm_goals/communications/proc/blackout_warning(blackout_start)
...
/datum/cm_goals/establish_power/get_point_value()
	return complete * priority

/datum/cm_goals/establish_power/total_point_value()
	return priority


/datum/cm_goals/retrieve_item/fulton
	name = "Restore Colony Communications"
...
/datum/cm_goals/retrieve_item/fulton/get_clue()
	return SPAN_DANGER("Retrieve lost fulton of [target_item] in [initial_location]")


/datum/cm_goals/retrieve_item/fulton/fail()
	. = ..()
	//Objective is failed, doesn't need to be here anymore
	qdel(src)

/datum/cm_goals/retrieve_item/fulton/complete()
	. = ..()
