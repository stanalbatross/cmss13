...
/datum/objective_memory_storage
	var/list/datum/cm_goals/folders = list()
	var/list/datum/cm_goals/progress_reports = list()
	var/list/datum/cm_goals/technical_manuals = list()
	var/list/datum/cm_goals/terminals = list()
	var/list/datum/cm_goals/disks = list()
	var/list/datum/cm_goals/retrieve_items = list()
	var/list/datum/cm_goals/other = list()
...
/datum/objective_memory_storage/proc/store_objective(var/datum/cm_goals/O)
	for(var/datum/cm_goals/R in O.enables_objectives)
		store_single_objective(R)

/datum/objective_memory_storage/proc/store_single_objective(var/datum/cm_goals/O)
	if(!istype(O))
		return
	if (O.is_finalised())
		//We don't need to store the objective, it is either completed or failed and won't be coming back
		return
	if(istype(O, /datum/cm_goals/document/folder))
		addToListNoDupe(folders, O)
	else if(istype(O, /datum/cm_goals/document/progress_report))
		addToListNoDupe(progress_reports, O)
	else if(istype(O, /datum/cm_goals/document/technical_manual))
		addToListNoDupe(technical_manuals, O)
	else if(istype(O, /datum/cm_goals/retrieve_data/terminal))
		addToListNoDupe(terminals, O)
	else if(istype(O, /datum/cm_goals/retrieve_data/disk))
		addToListNoDupe(disks, O)
	else if(istype(O, /datum/cm_goals/retrieve_item))
		addToListNoDupe(retrieve_items, O)
	else
		addToListNoDupe(other, O)

//returns TRUE if we have the objective already
/datum/objective_memory_storage/proc/has_goal(var/datum/cm_goals/O)
	if(O in folders)
		return TRUE
...
/datum/objective_memory_storage/proc/view_goals_memories(mob/recipient, var/real_name)
	synchronize_objectives()
	var/output

	output += "[goals_controller.check_goals_percentage()]%"
	...
	output += SSgoals.get_objectives_progress()
...
/datum/objective_memory_storage/proc/format_objective_list(var/list/datum/cm_goals/os, var/category)
	var/output = ""
	if (!os || !os.len)
		return output

	var/something_to_display = FALSE
	for(var/datum/cm_goals/O in os)
		if(!O)
			continue
...
/datum/objective_memory_storage/proc/clean_objectives()
	for(var/datum/cm_goals/O in folders)
		if(O.is_finalised())
			folders -= O
	for(var/datum/cm_goals/O in progress_reports)
		if(O.is_finalised())
			progress_reports -= O
	for(var/datum/cm_goals/O in technical_manuals)
		if(O.is_finalised())
			technical_manuals -= O
	for(var/datum/cm_goals/O in terminals)
		if(O.is_finalised())
			terminals -= O
	for(var/datum/cm_goals/O in disks)
		if(O.is_finalised())
			disks -= O
	for(var/datum/cm_goals/O in retrieve_items)
		if(O.is_finalised())
			retrieve_items -= O
	for(var/datum/cm_goals/O in other)
		if(O.is_finalised())
			other -= O
...
/datum/objective_memory_storage/proc/synchronize_objectives()
	...
	for(var/datum/cm_goals/O in intel_system.oms.folders)
		addToListNoDupe(folders, O)
	for(var/datum/cm_goals/O in intel_system.oms.progress_reports)
		addToListNoDupe(progress_reports, O)
	for(var/datum/cm_goals/O in intel_system.oms.technical_manuals)
		addToListNoDupe(technical_manuals, O)
	for(var/datum/cm_goals/O in intel_system.oms.terminals)
		addToListNoDupe(terminals, O)
	for(var/datum/cm_goals/O in intel_system.oms.disks)
		addToListNoDupe(disks, O)
	for(var/datum/cm_goals/O in intel_system.oms.retrieve_items)
		addToListNoDupe(retrieve_items, O)
	for(var/datum/cm_goals/O in intel_system.oms.other)
		addToListNoDupe(other, O)
...
/datum/intel_system/proc/store_objective(var/datum/cm_goals/O)
	oms.store_objective(O)

/datum/intel_system/proc/store_single_objective(var/datum/cm_goals/O)
...
/obj/structure/machinery/computer/intel/attack_hand(mob/living/user)
	...
	for(var/datum/cm_goals/O in user.mind.objective_memory.folders)
	...
	for(var/datum/cm_goals/O in user.mind.objective_memory.progress_reports)
		outcome = transfer_intel(user, O)
		if(outcome < 0)
			return FALSE
		if(outcome > 0)
	...
	for(var/datum/cm_goals/O in user.mind.objective_memory.technical_manuals)
		outcome = transfer_intel(user, O)
		if(outcome < 0)
			return FALSE
		if(outcome > 0)
	...
	for(var/datum/cm_goals/O in user.mind.objective_memory.terminals)
		outcome = transfer_intel(user, O)
		if(outcome < 0)
			return FALSE
		if(outcome > 0)
	...
	for(var/datum/cm_goals/O in user.mind.objective_memory.disks)
		outcome = transfer_intel(user, O)
		if(outcome < 0)
			return FALSE
		if(outcome > 0)
	...
	for(var/datum/cm_goals/O in user.mind.objective_memory.retrieve_items)
		outcome = transfer_intel(user, O)
		if(outcome < 0)
			return FALSE
		if(outcome > 0)
	...
	for(var/datum/cm_goals/O in user.mind.objective_memory.other)
		outcome = transfer_intel(user, O)
		if(outcome < 0)
			return FALSE
		if(outcome > 0)
...
/obj/structure/machinery/computer/intel/proc/transfer_intel(mob/living/user, var/datum/cm_goals/O)
	if(!intel_system || !intel_system.oms)
		return 0
	if(intel_system.oms.has_goal(O))
		return 0
	if(user.action_busy)
		return 0
