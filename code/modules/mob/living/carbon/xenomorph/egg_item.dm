...
/obj/item/xeno_egg/Initialize(mapload, datum/faction_status/hive_to_get)
	pixel_x = rand(-3,3)
	pixel_y = rand(-3,3)
	create_reagents(60)
...
/obj/item/xeno_egg/examine(mob/user)
	...
	if(isXeno(user))
		...
		if(faction != user.faction)
			to_chat(user, "This one appears to belong to the [faction.prefix]hive")
...
/obj/item/xeno_egg/proc/plant_egg(mob/living/carbon/Xenomorph/user, turf/T, proximity = TRUE)
	...
	for(var/obj/effect/alien/weeds/W in T)
		if(W.weed_strength >= WEED_LEVEL_HIVE && W.faction == faction)
			hive_weeds = W
			break
...
/obj/item/xeno_egg/alpha
	color = "#ff4040"
	faction = SET_FACTION_HIVE_ALPHA