...
/datum/resin_construction/proc/can_build_here(var/turf/T, var/mob/living/carbon/Xenomorph/X, var/blocker_ignore = FALSE)
	var/mob/living/carbon/Xenomorph/blocker = locate() in T
	if(blocker && blocker != X && blocker.stat != DEAD && blocker_ignore == FALSE)
...
/datum/resin_construction/resin_obj/pillar/build(var/turf/T, var/hivenumber, var/time_to_brittle, var/time_to_collapse)
	var/obj/structure/resin_pillar/P = ..()
	if(time_to_brittle && time_to_collapse)
		P.start_decay(time_to_brittle, time_to_collapse)
	else
		P.start_decay()
	return P
...
// TECHWEBS - Resin Pillar

/datum/resin_construction/resin_obj/pillar
	name = "Resin Pillar"
	construction_name = "resin pillar"
	cost = XENO_RESIN_PILLAR_COST
	build_time = 4.5 SECONDS
	obj_path = /obj/structure/resin_pillar
