		...
		if(target.stat != DEAD)
			if(target.health < target.maxHealth)
				if(X.check_plasma(plasma_cost))
					...
					target.visible_message(SPAN_BOLDNOTICE("You feel a presence surrounding [target], and it spasms as its wounds quickly seal up!"))	//marines probably should know if a xeno gets healed
					target.xeno_jitter(SECONDS_2)
					target.flick_heal_overlay(SECONDS_3, "#D9F500")	//it's already hard enough to gauge health without hp overlays!
			else
