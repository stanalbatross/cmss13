...
/mob/living/carbon/human/update_med_icon()
	...
	for(var/obj/limb/L in limbs)
		for(var/obj/item/stack/medical/M in L.medical_items)
			if(!M.cached_limb_icon)
				M.cached_limb_icon = icon('icons/mob/humans/onmob/med_human.dmi', "[M.icon_state_prefix]_[L.name]")
			standing_image.overlays += M.cached_limb_icon
