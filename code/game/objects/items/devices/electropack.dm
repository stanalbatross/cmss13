...
/obj/item/device/electropack/attack_hand(mob/user as mob)
	if(src == user.back)
		to_chat(user, SPAN_NOTICE("You need help taking this off!"))
		return
	..()

/obj/item/device/electropack/interact(mob/user, flag1)
	if(!secured)
...
/obj/item/device/electropack/tgui_interact(mob/user, datum/tgui/ui)
	ui = SStgui.try_update_ui(user, src, ui)
	if (!ui)
		ui = new(user, src, "Electropack", "Electropack")
		ui.open()
		ui.set_autoupdate(FALSE)

/obj/item/device/electropack/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	. = ..()
...
/obj/item/device/electropack/ui_data(mob/user)
	. = list()

	.["current_freq"] = frequency

/obj/item/device/electropack/ui_static_data(mob/user)
	. = list()
...
/obj/item/device/electropack/attack_self(mob/user as mob, flag1)
	..()
