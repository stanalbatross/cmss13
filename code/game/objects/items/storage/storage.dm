...
/obj/item/storage/proc/dump_ammo_to(obj/item/ammo_magazine/ammo_dumping, mob/user)
	if(user.action_busy)
		return

	if(ammo_dumping.flags_magazine & AMMUNITION_HANDFUL_BOX)
		var/handfuls = round(ammo_dumping.current_rounds / 5, 1) //The number of handfuls, we round up because we still want the last one that isn't full
			...
			if(contents.len < storage_slots)
				...
				for(var/i = 1 to handfuls)
					if(contents.len < storage_slots)
						...
						var/MR = ammo_dumping.caliber == "12g" ? 5 : 8
						R = 5 ? min(ammo_dumping.current_rounds, 5) : min(ammo_dumping.current_rounds, MR)
						new_handful.generate_handful(ammo_dumping.default_ammo, ammo_dumping.caliber, MR, R, ammo_dumping.gun_type)
						ammo_dumping.current_rounds -= R
