...
/datum/element/drop_retrieval/mister/dropped(obj/item/I, mob/user)
	...
	if(!istype(WT))
		return ..()
	WT.remove_noz()
	to_chat(user, SPAN_WARNING("The [I]'s magnetic harness snaps it back onto [WT]"))
	var/obj/item/reagent_container/spray/mister/atmos/AT = WT.noz
	if(istype(AT))
		WT.icon_state = "backpack_foamer"
